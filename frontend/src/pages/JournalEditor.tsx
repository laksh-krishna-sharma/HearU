...
const JournalEditor: React.FC = () => {
  ...
  useEffect(() => {
    if (isEditing && id) {
      dispatch(getJournal(id)).then((res: any) => {
        if (res.payload) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    const entry = { title, content, tags };
          ...
          <div className="lg:col-span-2">
            <Card className="h-full">
              ...
              <CardContent className="space-y-4">
                <Input placeholder="Enter journal title..." value={title} onChange={(e) => setTitle(e.target.value)} className="text-lg font-semibold" />
                <Textarea placeholder="Write your journal entry here..." value={content} onChange={(e) => setContent(e.target.value)} className="min-h-96 resize-none" />
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {tags.map((tag, idx) => (
                      <span key={idx} className="flex items-center gap-1 bg-ocean-primary text-white px-2 py-1 rounded-full text-sm">
                        {tag}
                        <button type="button" className="text-white hover:text-gray-200" onClick={() => setTags(tags.filter((t) => t !== tag))}>×</button>
                      </span>
                    ))}
                  </div>
                  <input
