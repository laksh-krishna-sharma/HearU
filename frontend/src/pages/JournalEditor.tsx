...
const JournalEditor: React.FC = () => {
    ...
    const playAudio = (audioPath: string, isIntro: boolean = false) => {
      ...
      audioPlayer.onended = () => {
        ...
        if (isIntro) {
          ...
          setTimeout(() => dispatch(clearJournalReply()), 1000);
        }
      };
  ...
  const handleGetJournalReply = async () => {
    ...
    if (session) dispatch(resetEveState());
  ...
  const handleStartRecording = async () => {
    if (!session || isRecording || isIntroPlaying) {
      if (isIntroPlaying) toast('Please wait for Eve to finish speaking before recording.');
      return;
    }
    try {
  ...
  const handleStartSession = async () => {
    if (journalReply) dispatch(clearJournalReply());
    dispatch(clearError());
  ...
  const handleEndSession = async () => {
    if (session?.session_id) setShowSaveModal(true);
  };
  ...
  const handleConfirmEndSession = async (save_summary: boolean) => {
    ...
    try {
      ...
      if (save_summary && id && isEditing) endSessionPayload.journal_id = id;
  ...
  if (journalNotFound) return <NotFoundView onBack={() => navigate('/journal')} />;
      ...
      <div className="max-w-8xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0">
          ...
          <div className="flex flex-col h-full gap-6 min-h-0">
            <div className="basis-1/2 min-h-0 overflow-auto w-full max-w-full">
              <div className="w-full max-w-full">
                <NotesPanel voiceSummaries={voiceSessionResponses} />
              </div>
            </div>

            <div className="basis-1/2 min-h-0 overflow-y-auto overflow-x-hidden w-full max-w-full">
              <div className="w-full max-w-full">
                <VoiceAssistantPanel
                  session={session}
