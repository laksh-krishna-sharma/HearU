...
async def register(req: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    return {"message": "User registered successfully", "user": UserOut(**user.to_dict())}
...
async def logout(current_user: User = Depends(get_current_user)):
    ...
    if not jti:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Token jti not available")
    revoke_jti(jti)
...
async def list_users(admin: User = Depends(admin_required), db: AsyncSession = Depends(get_db)):
