...
from routes.chat.schema.chat import (
...
async def create_session(payload: CreateSessionRequest, current_user: User = Depends(get_current_user)):
...
async def get_session(session_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    items = [MessageOut(id=m.id, role=m.role.value if hasattr(m.role, "value") else str(m.role), content=m.content, created_at=m.created_at.isoformat()) for m in msgs]
...
async def send_message(session_id: int, payload: SendMessageRequest, current_user: User = Depends(get_current_user)):
    cs = await _chat_service.get_session(session_id)
    if not cs:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="session not found")
    if cs.user_id and cs.user_id != current_user.id and not current_user.is_admin:
...
async def agent_endpoint(payload: AgentRequest, current_user: User = Depends(get_current_user)):
